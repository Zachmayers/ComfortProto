...
class Member {
	...
	function get_member_data() {
		...
		$database->query('SELECT userID, email, firstname, lastname, profile_status, type, creation_date, profile_status, email_validation, email_setting, valid, last_login, photo_setting, zip, quote, description, valid_hash, ref_jobID, public
									FROM members 
									WHERE userID = :userID');
	...
	function update_member_data($type, $member_update_array) {
		if ($_SESSION['userID'] == $this->userID && count($member_update_array) > 0) {
			$database = new Database;			
				switch($type) {
					case "general":
						$database->query('UPDATE members 
													...
													WHERE userID = :userID');
						$database->bind(':userID', $this->userID);
						$database->bind(':firstname', $member_update_array['firstname']);
						$database->bind(':lastname', $member_update_array['lastname']);
						$database->bind(':zip', $member_update_array['zip']);
						$database->bind(':contact_phone', $member_update_array['contact_phone']);
						$database->execute();											
					break;	
					
					case "employer_general":
						$database->query('UPDATE members 
													SET firstname = :firstname,
															lastname = :lastname
													WHERE userID = :userID');
					...
					case "phone":
						$database->query('UPDATE members 
													...
													WHERE userID = :userID');
						$database->bind(':userID', $this->userID);
						$database->bind(':contact_phone', $member_update_array['phone']);
						$database->execute();											
					break;	
