...
$(document).on('click', '#learn_button', function(event){
	...
	$("#login_button").click(function() {
		...
		user = $('#user_login').val();
		pass = $('#pass_login').val();
		dataString = "user=" + user + "&pass=" + encodeURIComponent(pass);
		//alert(dataString);							
		$("#delay_holder").empty();					
		$('#login_button').hide();		
		$('#fb_holder').hide();						
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data == "false") {
					...
					$('#login_button').show();																																			
					$('#fb_holder').show();																			
				} else if (data == "complete" || data == "admin") {			
					window.location = "main.php";
				} else {
	...
	$("#blark").unbind('click').click(function() {
			$(".warning").hide();
			...
			dataString = "first=" + encodeURIComponent(first) + "&last=" + encodeURIComponent(last) + 
								"&login="  + login_email + "&pass=" + encodeURIComponent(set_password) + 
								"&display_name=" + encodeURIComponent(display_name);
//alert(dataString);
			if (first.length == 0 || last.length == 0 || login_email.length == 0 || set_password.length == 0 || display_name.length == 0) {
				$('#loader').hide();
				$('#sign_up_form').show();
				...
				$.ajax({
					type: "POST",
					url: "ajax/verification.ajax.php?type=register_member",
					data: dataString,
					success: function(data) {
						//alert(data);
						if (data == "email") {
...
//	alert(public_hash);
	    ...
	    FB.getLoginStatus(function(response) {
	        if (response.status === 'connected') {
	            get_user_details();
		         ...
		         //alert("we should be here, login is next step");
  ...
  function get_user_details() {
	...
	$("#fb_connect").click(function(evt) {
		...
		fb_ID = $('#fb_ID').val();
/*
		email = $('#connect_email').attr('value').trim();
		password = $('#connect_pass').attr('value').trim();
*/
