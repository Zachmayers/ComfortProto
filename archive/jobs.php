...
*/
	...
	$job_list = $opportunity_list->get_public_job_list_new($region, $position, $page);

	jobs_header_new_html($site_type, $meta_data, $region, $js_file, $position);
	
//	jobs_html($region_text, $public_job_slots, $today);	
	jobs_html_new($region_text, $job_list, $today, $region, $position);	
...
<script>
	$(document).ready(function() {
		page = <? echo $page ?>;
		region = "<? echo $_GET['region'] ?>";
		position = "<? echo $_GET['position'] ?>";

		new_jobs(page, region, position)					
	})
</script>
<?php
